...
do {
   $url = 'http://ca.megabus.com/JourneyResults.aspx?originCode='.$fromCity.'&destinationCode='.$toCity.'&outboundDepartureDate='.date('d/m/y', $startDate).'&inboundDepartureDate=&passengerCount=1&transportType=0&concessionCount=0&nusCount=0&outboundWheelchairSeated=0&outboundWheelchairStowed=0&inboundWheelchairSeated=0&inboundWheelchairStowed=0&outboundPcaCount=0&inboundPcaCount=0&promotionCode='.$promoCode.'&withReturn=0';
	...
	for ($x=0; $x<$i; $x++) {
		...
		echo '<td>'.$price[$x].'</td>';
		echo '<td><a href="http://ca.megabus.com/'.$link[$x].'">View</a><div class="view"><iframe src="http://ca.megabus.com/'.$link[$x].'"></iframe></div></td>';
		echo '</tr>';
	}
